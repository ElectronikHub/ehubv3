...
useEffect(() => {
  const fetchProducts = async () => {
    try {
      const res = await api.get('/', { params: { sort: sortOption } });

      // 👇 Check what the structure of the response looks like
      console.log("Full API response:", res.data);

      // 👇 Use res.data.data if Laravel wraps your product list
      const products = res.data.data; // Update this line based on actual response
      ...
      const productsWithImages = await Promise.all(
        products.map(async (product) => {
          try {
            ...
            console.error(
              `Image fetch failed for product ${product.id}`,
              imageError
            );
            return { ...product, image: '/img/default.png' }; // Note: fixed fallback key
          }
        })
      );
