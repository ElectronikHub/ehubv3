...
const Products = () => {
  ...
  const categories = useMemo(() => ({
    ALL: [],
    SENSORS: [
      'Light/Color/Imaging', 'MOTION', 'Temperature & Humidity', 'Flex/Force', 'Weather Sensors',
      'Wifi', 'Antennas', 'RF', 'Gas', 'Proximity/Distance/Range', 'GPS', 'Sound/Noise',
      'Liquid/Flow', 'GSM', 'Biometrics', 'Bluetooth'
    ],
    COMPONENTS: [],
    MICROCONTROLLERS: ['Arduino', 'RASPBERRY PI ZERO'],
  }), []);
 ...
 useEffect(() => {
  const fetchProducts = async () => {
    try {
      const res = await api.get('/'); // baseURL + '/' = http://localhost:8000/api/apiproducts/
  ...
  useEffect(() => {
    ...
    const result = allProducts
      .filter((product) => {
        ...
        return product.type?.toLowerCase() === normalizedType;
      })
      .filter((product) =>
    ...
    const allCategories = Object.entries(categories).flatMap(([group, subs]) =>
      subs.length ? subs.map((sub) => `${group} > ${sub}`) : [group]
    );
  ...
  }, [selectedType, searchQuery, sortOption, categories, allProducts]);
                  ...
                  {categorySuggestions.length === 0 ? (
                    ...
                    categorySuggestions.map((cat, idx) => (
                      <div
                        key={idx}
                        onMouseDown={() => {
                          const [group, sub] = cat.split(' > ');
                          setSelectedType(sub || group);
                          setSearchQuery('');
                          setIsSearchFocused(false);
                        }}
                        className="px-4 py-2 hover:bg-gray-100 cursor-pointer"
              ...
              {Object.entries(categories).map(([group, subcats]) => (
                <div key={group} className="mb-2">
                  <button
                    ...
                    className="w-full text-left flex justify-between items-center font-semibold text-sm hover:text-primary transition-colors"
                  >
                    {group}
                    {subcats.length > 0 && (
                      <span className="ml-2">{expandedGroups[group] ? '−' : '+'}</span>
                    )}
                  </button>
                  {subcats.length > 0 && expandedGroups[group] && (
                    <div className="ml-2 mt-1 space-y-1">
                      {subcats.map((subcat) => (
                        <button
                          key={subcat}
                          onClick={() => {
                            setSelectedType(subcat);
                            setShowSidebar(false);
                          }}
                          className={`block w-full text-left text-black hover:text-primary text-xs ${
                            selectedType === subcat ? 'font-bold text-primary' : ''
                  ...
                  {subcats.length === 0 && (
                    <button
                      ...
                      className="mt-1 pl-3 text-xs hover:text-primary py-1 rounded transition-colors text-black"
                    >
                      View All
                    </button>
                  )}
